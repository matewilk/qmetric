...
module.exports.answerQuestions = (questionset, answers, id = '', output = []) => {
    questionset.map((question) => {
        ...
        if (R.keys(question).includes('groups')) {
            module.exports.drillDown(question.groups, answers, outputId, output);
        }
    });
...
module.exports.drillDown = (groups, answers, outputId, output) => {
    let answer = answers[outputId];
    let trigger = R.find(R.propEq('trigger', answer))(groups);
